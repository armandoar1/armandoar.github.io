...
<body>
  ...
  <?php
    ...
    $registro=$mysql->query("select descripcion from rubros where NUsuario=$_REQUEST[NUsuario]") or
      die($mysql->error);
	 
    if ($reg=$registro->fetch_array())
    {
      $mysql->query("delete from miembro where NUsuario=$_REQUEST[NUsuario]") or
        die($mysql->error);    
      echo 'El Nombre del Usuario que se eliminó es:'.$reg['Nombre'];	  
    }      
    else
